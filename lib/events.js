...
exports.updateEventIncomingQuestions = async (req, res) => {
    if (!req.permissions.edit_incoming_questions) {
        return errors.makeForbiddenError(res, 'You are not allowed to edit incoming questions for events of this type.');
    }

    const dbResult = await req.event.update({ incoming_questions: req.body.incoming_questions });
