...
exports.listEvents = async (req, res) => {
    ...
    if (req.query.all) {
        ...
        if (req.permissions.show_unpublished_event.agora) {
            allowedDrafts.push('agora');
        }

        if (req.permissions.show_unpublished_event.epm) {
            allowedDrafts.push('epm');
        }

        if (req.permissions.show_unpublished_event.spm) {
            allowedDrafts.push('spm');
        }

        if (!req.query.type) {
            // TODO: add subquery where so it's either status: published OR status: draft AND type in allowedDrafts
            query.where[Sequelize.Op.and].push({ status: 'published' }, { type: { [Sequelize.Op.in]: allowedDrafts } });
        }

        if (req.query.type) {
            // TODO: check the code below since it was generated by GitHub Copilot
            let allowedFiltered = [];
            if (Array.isArray(req.query.type)) {
                allowedFiltered = allowedDrafts.filter((allowedDraft) => req.query.type.includes(allowedDraft));
            } else {
                allowedFiltered = allowedDrafts.filter((allowedDraft) => allowedDraft === req.query.type);
            }

            if (allowedFiltered.length > 1) {
                // TODO: add subquery where so it's either status: published OR status: draft AND type in allowedFiltered
                query.where[Sequelize.Op.and].push({ status: 'published' }, { type: { [Sequelize.Op.in]: allowedFiltered } });
            }

            if (allowedFiltered.length === 1) {
                // TODO: add subquery where so it's either status: published OR status: draft AND type: allowedFiltered
                query.where[Sequelize.Op.and].push({ status: 'published' }, { type: allowedFiltered });
            }
