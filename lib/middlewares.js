...
exports.fetchSingleApplication = async (req, res, next) => {
    ...
    const userPrefix = req.params.application_id === constants.CURRENT_USER_PREFIX ? 'You haven\'t' : 'This user hasn\'t';

    const application = await Application.findOne({ where: whereObj });
    if (!application) {
        return errors.makeNotFoundError(res, `${userPrefix} applied to this event yet.`);
    }
    ...
    // TOOD: ensure that on the frontend the email is only shown if this permission is true
    if (req.permissions.see_user_email) {
        const mail = await core.getMails(req, application.user_id);

        if (mail) {
            req.application.dataValues.notification_email = mail[0].notification_email;
        }
    }
