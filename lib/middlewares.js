...
const { Event, Application, Incoming, Image, PaxLimit } = require('../models');
...
exports.fetchSingleIncoming = async (req, res, next) => {
    const whereObj = { event_id: req.event.id };
    ...
    const incoming = await Incoming.findOne({ where: whereObj });
    if (!incoming) {
        return errors.makeNotFoundError(res, userPrefix + ' haven\'t applied to this event yet.');
    }
    ...
    req.permissions = helpers.getIncomingPermissions({
        permissions: req.permissions,
        corePermissions: req.corePermissions,
        event: req.event,
        mine: req.user.id === req.incoming.user_id
    });
