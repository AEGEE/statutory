...
exports.postApplication = async (req, res) => {
    ...
    const also_subscribe_listserv = req.body.listserv;

    if (also_subscribe_listserv){

      await mailer.sendMail({
          to: config.listserv_email,
          from: req.user.notification_email,
          subject: `SUBSCRIBE ${req.user.notification_email}`, //NOTE: the from MUST be from the subscriber; subject's not important
          template: 'listserv.html',
          /*
          OK BEGIN
          REG ${first_name} ${last_name} (${req.body.body_name})
          SUBSCRIBE AEGEE-L
          SUBSCRIBE AEGEE-EVENT-L
          SUBSCRIBE ANNOUNCE-L
          SUBSCRIBE AEGEENEWS-L
          OK END
          */
          parameters: {
              name: req.body.first_name,
              surname: req.body.last_name,
              antenna: req.body.body_name
          }
      });
    }
