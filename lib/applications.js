...
exports.listAllApplications = async (req, res) => {
    if (!req.permissions.see_applications || !req.permissions.see_user_email) {
...
exports.listIncomingApplications = async (req, res) => {
    if (!req.permissions.see_applications_incoming || !req.permissions.see_user_email) {
        return errors.makeForbiddenError(res, 'You are not allowed to see applications.');
    }

    const attributesObj = constants.ALLOWED_INCOMING_FIELDS.filter((field) => field !== 'notification_email');
...
exports.listBoardView = async (req, res) => {
    ...
    const applications = await Application.findAll({
        ...
        attributes: ['statutory_id', 'first_name', 'last_name', 'participant_type', 'participant_order', 'is_on_memberslist', 'status', 'cancelled', 'board_comment']
    });
...
exports.exportOpenslides = async (req, res) => {
    ...
    if (!req.permissions.export.openslides || !req.permissions.see_user_email) {
        return errors.makeForbiddenError(res, 'You are not allowed to export for OpenSlides.');
    }
...
exports.exportAll = async (req, res) => {
    ...
    if (req.query.select.includes('notification_email') && applications.length > 0) {
        if (!req.permissions.see_user_email) {
            return errors.makeForbiddenError(res, 'You are not allowed to export participants\' email.');
        }
...
exports.exportDelegatesJc = async (req, res) => {
    ...
    if (!req.permissions.export.delegates_jc || !req.permissions.see_user_email) {
