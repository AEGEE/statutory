...
exports.getMemberslistAmounts = async (req, res) => {
    if (Number.isNaN(parseInt(req.params.body_id, 10))) {
        return errors.makeBadRequestError(res, 'The body_id parameter is invalid.');
    }

    const memberslists = await MembersList.findAll({ where: {
        body_id: parseInt(req.params.body_id, 10)
    }})

    if (!memberslists) {
        return errors.makeNotFoundError(res, 'Members lists are not found.');
    }

    data = memberslists.map(memberslist => ({
        id: memberslist.id,
        event_id: memberslist.event_id,
        members: memberslist.members.length,
    }));
