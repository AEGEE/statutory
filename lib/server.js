...
const IncomingRouter = router({ mergeParams: true });
const SingleIncomingRouter = router({ mergeParams: true });
...
EventsRouter.post('/incoming_questions', events.updateEventIncomingQuestions);
...
SingleApplicationRouter.put('/incoming_form', applications.updateApplicationIncoming);
SingleApplicationRouter.get('/', applications.getApplication);
SingleApplicationRouter.put('/', applications.updateApplication);

IncomingRouter.use(middlewares.authenticateUser, middlewares.ensureAuthorized, middlewares.fetchEvent);
IncomingRouter.post('/', incoming.postIncoming);
IncomingRouter.get('/all', incoming.listAllIncoming);

SingleIncomingRouter.use(middlewares.authenticateUser, middlewares.ensureAuthorized, middlewares.fetchEvent, middlewares.fetchSingleIncoming);
SingleIncomingRouter.get('/', incoming.getIncoming);
SingleIncomingRouter.put('/', incoming.updateIncoming);
...
server.use('/events/:event_id/incoming', IncomingRouter);
server.use('/events/:event_id/incoming/:incoming_id', SingleIncomingRouter);
